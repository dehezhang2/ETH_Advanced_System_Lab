...
void kernel_base(complex_t* A, complex_t* x, complex_t* y, int n) {
    complex_t t, sum;
    for (int i = 0; i < n; i++) {
        sum = {0.0, 0.0};
        for (int j = 0; j < n; j++) {
...
double perf_test(comp_func f, string desc, int flops) {
    ...
    int n = 64;
    complex_t *A, *x, *y;
    build((double**) &A, 2*n, n);
    build((double**) &x, 2*n, 1);
    build((double**) &y, 2*n, 1);
...
int main(int argc, char **argv) {
  ...
  int n = 64;
  complex_t *A, *x, *y, *y_base, *y_copy;
  build((double**) &A, 2*n, n);
  build((double**) &x, 2*n, 1);
  build((double**) &y, 2*n, 1);
  build((double**) &y_base, 2*n, 1);
  build((double**) &y_copy, 2*n, 1);
  copy_n(y, n, y_base);
  copy_n(y, n, y_copy);
  
  kernel_base(A, x, y_base, n);

  for (i = 0; i < numFuncs; i++) {
    comp_func f = userFuncs[i];
    f(A, x, y, n);
    
    double error = nrm_sqr_diff((double*) y, (double*) y_base, 2*n);
    if (error > EPS) {
      cout << error << endl;
